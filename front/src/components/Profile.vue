<template>
    <div class="top">
        <i class="el-icon-a-01" style="font-size: 32px;">
            <b>当前登录信息</b>
        </i>
        ...
        <el-descriptions title="" :column="1" size="40" border style="padding: 20px 0 20px 0">
            ...
            <el-descriptions-item>
                <template slot="label">
                    <i class="iconfont icon-r-mark1"></i>角色
                </template>
                <el-tag type="success" disable-transitions>{{ user.roleId == 0 ? "超级管理员" : user.roleId == 1 ? "管理员" :
                    "用户" }}</el-tag>
            </el-descriptions-item>
        </el-descriptions>
        ...
        <el-row style="text-align: left">
            <el-button type="danger" @click="logout" style="margin: 25px 0 0 90px; font-size: 22px;">
                <i class="iconfont icon-r-no" style="font-size: 24px;"></i>
                退出登录
            </el-button>
        </el-row>
    </div>
